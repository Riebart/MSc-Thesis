...
the result applies. By further restricting that the queries follow a prescribed
character distribution, there are statistical properties that the norms of the
queries in $\mathbb E^n$ will have. By observing that queries can be considered
(Independent Identically Distributed) IID observations from a fixed alphabet,
$\mathcal C=\{c_i\}$, with a known probability distribution, $\forall
c\in\mathcal C, 0\leq P(c)\leq1$ satisfying $\sum_{c\in\mathcal C}{P(c)}=1$, the
notion of a typical set can be applied. As $n$ (the length of a query/the
dimension of the vector) grows, a typical sequence can be assumed to have each
symbol $c\in\mathcal C$ appearing $nP(c)$ times. If $v\in\mathcal C^n$ is a
vector with the Euclidean $n$-norm, then asymptotically

\[\|\mathbf v\|\approx k\sqrt n E[(\mathcal C,P)] \]
...
$\mathcal C^n$ becomes very tightly packed around the norm of a typical vector.
This is a consequence of the \emph{Central Limit Theorem} since the components
of the vectors (characters in the DNS query) are IID, and a variance that
decreases as the inverse of the dimensionality.
